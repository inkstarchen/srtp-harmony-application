...
// Forward declarations for static plugin registration functions
// These are generated by DEFINE_STATIC_PLUGIN macro in registry_data.cpp
...
bool LoadFunc(T &fn, const char *fName, void *handle)
{
    ...
    fn = converter.fnPtr;
    if (fn == nullptr) {
        WIDGET_LOGE("%{public}s open %{public}s", __func__, dlerror());
        return false;
    }
    return true;
}
...
bool LumeCommon::LoadEngineLib()
{
#if defined(CORE_DYNAMIC) && (CORE_DYNAMIC == 1)
    if (libHandle_ != nullptr) {
        WIDGET_LOGE("%{public}s, already loaded", __func__);
        return false;
    }
    ...
    if (libHandle_ == nullptr) {
        WIDGET_LOGE("%{public}s, open lib fail %{public}s", __func__, dlerror());
    }
    #undef TO_STRING
    #undef LIB_NAME
...
bool ActivateProductBasis(const std::string &bundleName)
{
    ...
    WIDGET_LOGD("ActivateProductBasishapPath: %{public}s render config %{public}s", bundleName.c_str(), renderConfig.c_str());
...
#endif
    WIDGET_LOGI("LUMECOMMON: ENGINE INIST SUCCESS");
    return engine_;
}
...
{
    WIDGET_LOGI("CreateRenderContext: Start creating render context");

    // Manually register static plugins since constructor may not be called
    // when using static libraries with $<TARGET_OBJECTS:...>
    auto& pluginRegistry = CORE_NS::GetPluginRegister();

    WIDGET_LOGI("CreateRenderContext: Manually registering static plugins");

    WIDGET_LOGI("CreateRenderContext: Calling AGPRender_RegisterStaticPlugin()");
    AGPRender_RegisterStaticPlugin();
    WIDGET_LOGI("CreateRenderContext: AGPRender plugin registered");

    WIDGET_LOGI("CreateRenderContext: Calling AGP3D_RegisterStaticPlugin()");
    AGP3D_RegisterStaticPlugin();
    WIDGET_LOGI("CreateRenderContext: AGP3D plugin registered");

    WIDGET_LOGI("CreateRenderContext: Static plugins registered");

    WIDGET_LOGI("CreateRenderContext: Loading 3D plugins...");
    ...
    WIDGET_LOGI("CreateRenderContext: Requested plugin count: %zu", sizeof(uid3dplugins) / sizeof(uid3dplugins[0]));

    bool loadResult = pluginRegistry.LoadPlugins(uid3dplugins);
    WIDGET_LOGI("CreateRenderContext: LoadPlugins returned: %d", loadResult);

    if (!loadResult) {
        WIDGET_LOGE("CreateRenderContext: Failed to load 3D plugins");
    } else {
        WIDGET_LOGI("CreateRenderContext: 3D plugins loaded successfully");
    }
    ...
    // Check engine_ validity
    if (!engine_) {
        WIDGET_LOGE("CreateRenderContext: engine_ is nullptr!");
        return nullptr;
    }
    WIDGET_LOGI("CreateRenderContext: engine_ is valid");

    // Get class factory
    auto classFactory = engine_->GetInterface<Core::IClassFactory>();
    if (!classFactory) {
        WIDGET_LOGE("CreateRenderContext: Failed to get IClassFactory from engine");
        return nullptr;
    }
    WIDGET_LOGI("CreateRenderContext: IClassFactory obtained successfully");

    // Create render context
    WIDGET_LOGI("CreateRenderContext: Creating IRenderContext instance...");
    renderContext_ = CORE_NS::CreateInstance<RENDER_NS::IRenderContext>(*classFactory, RENDER_NS::UID_RENDER_CONTEXT);

    if (renderContext_ == nullptr) {
        WIDGET_LOGE("lume Create render context fail");
        WIDGET_LOGE("CreateRenderContext: CORE_NS::CreateInstance returned nullptr for UID_RENDER_CONTEXT");
        return nullptr;
    }
    WIDGET_LOGI("CreateRenderContext: IRenderContext instance created successfully");
    ...
    WIDGET_LOGI("CreateRenderContext: Configuring GLES backend, sharedContext=%p", gfxContext);
    ...
    WIDGET_LOGI("CreateRenderContext: Initializing render context...");
    auto ret = renderContext_->Init(renderCreateInfo);
    if (ret != RENDER_NS::RenderResultCode::RENDER_SUCCESS) {
        WIDGET_LOGE("lume Init render context fail, error code: %d", static_cast<int>(ret));
        return nullptr;
    }
    WIDGET_LOGI("CreateRenderContext: Render context initialized successfully");
    ...
    CORE_NS::Entity &importedEntity, BASE_NS::vector<CORE3D_NS::GLTFResourceData> &res)
{
    ...
    LOGI("Destroy scene node and res");
    ...
    const GltfImportInfo &info, CORE_NS::Entity &importedEntity, BASE_NS::vector<CORE3D_NS::GLTFResourceData> &res)
{
    ...
    if (!gltf.success) {
        WIDGET_LOGE("LoadAndImport() Loaded '%{public}s' with errors:\n%{public}s", info.fileName_, gltf.error.c_str());
        return false;
    }
    if (!gltf.data) {
        WIDGET_LOGE("LoadAndImport gltf data is null. Error: %{public}s ", gltf.error.c_str());
        return false;
    }
    ...
    if (!gltfImportResult.success) {
        WIDGET_LOGE("LoadAndImport() Importing of '%{public}s' failed: %{public}s", info.fileName_, gltfImportResult.error.c_str());
        return false;
    }
...
void LumeCommon::CreateEcs(uint32_t key)
{
    ...
    WIDGET_LOGD("LumeCommon::CreateEcs() - key: %{public}d", key);
    key_ = key;
...
void LumeCommon::LoadSystemGraph(BASE_NS::string sysGraph)
{
    ...
    if (!result.success) {
        WIDGET_LOGE("load system graph %{public}s error %{public}s", sysGraph.c_str(), result.error.c_str());
    }
...
void LumeCommon::LoadSceneModel(const std::string &modelPath)
{
    ...
    LOGI("UNLoadSceneModel");
    UnloadSceneModel();
    LOGI("CreateSceneModel");
    CreateScene();
    GltfImportInfo file{
...
void DestroyNode(CORE_NS::Entity &sceneEntity, std::shared_ptr<Geometry> &shape, CORE3D_NS::INodeSystem &nodeSystem)
{
    ...
    if (shapeNode) {
        ...
        WIDGET_LOGW("Failed to remove: %{public}s", shape->GetName().c_str());
    }
}
    ...
    CORE_NS::Entity &sceneEntity, std::shared_ptr<Geometry> &shape, CORE3D_NS::INodeSystem &nodeSystem)
{
    ...
    WIDGET_LOGW("Failed to Update: %{public}s", shape->GetName().c_str());
}
...
bool LumeCommon::CreateSwapchain(void *nativeWindow)
{
    ...
    if (hasSRGB) {
        ...
        if (eglSurface_ == EGL_NO_SURFACE) {
            ...
            LOGI("create srgb egl surface success");
        }
    }
    ...
    if (!hasSRGB) {
        ...
        if (eglSurface_ != EGL_NO_SURFACE) {
            LOGI("create linear egl surface success");
    ...
    LOGI("LumeCommon::CreateSwapchain - swapchainHandle_ valid=%d, handleType=%d, handle=%llx",
        static_cast<bool>(swapchainHandle_),
        static_cast<int>(swapchainHandle_.GetHandleType()),
        swapchainHandle_.GetHandle().id);
    return eglSurface_ != EGL_NO_SURFACE;
}

void LumeCommon::SetupCustomRenderTarget(const TextureInfo &info)
{
    LOGI("LumeCommon::SetupCustomRenderTarget - ecs_=%p, nativeWindow_=%p, textureId_=%u",
        ecs_.get(), info.nativeWindow_, info.textureId_);
    if (ecs_ == nullptr) {
    ...
    if (!cameraComponent) {
        LOGE("LumeCommon::SetupCustomRenderTarget - cameraComponent is NULL, cameraEntity_ invalid!");
        return;
    }
    auto *rhcManager = CORE_NS::GetManager<CORE3D_NS::IRenderHandleComponentManager>(ecs);
    if (!rhcManager) {
        LOGE("LumeCommon::SetupCustomRenderTarget - rhcManager is NULL!");
        return;
    }
    cameraComponent->customColorTargets.clear();
    CORE_NS::EntityReference imageEntity;
    LOGI("LumeCommon::SetupCustomRenderTarget - textureId_=%u, nativeWindow_=%p",
        info.textureId_, info.nativeWindow_);
    if (info.textureId_ == 0U && info.nativeWindow_)
    {
        LOGI("LumeCommon::SetupCustomRenderTarget - creating swapchain");
        // need check recreate window
        CreateSwapchain(info.nativeWindow_);
        LOGI("LumeCommon::SetupCustomRenderTarget - swapchainHandle_ valid=%d, handleType=%d, handle=0x%llx",
            static_cast<bool>(swapchainHandle_),
            static_cast<int>(swapchainHandle_.GetHandleType()),
            swapchainHandle_.GetHandle().id);
        imageEntity = GetOrCreateEntityReference(ecs.GetEntityManager(), *rhcManager, swapchainHandle_);
        LOGI("LumeCommon::SetupCustomRenderTarget - imageEntity valid=%d", static_cast<bool>(imageEntity));
    } else {
        LOGI("LumeCommon::SetupCustomRenderTarget - cannot create swapchain (textureId=%u, nativeWindow=%p)",
            info.textureId_, info.nativeWindow_);
...
//    LOGI("LumeCommon::SetupCustomRenderTarget - customColorTargets size=%zu, cameraEntity=%u",
//        cameraComponent->customColorTargets.size(), (uint32_t)cameraEntity_);
}
