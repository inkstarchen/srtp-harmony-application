...
NativeResourceManager* LumeXComponent::LumeXComponentManager::resourceMgr_ = nullptr;
...
ArkUI_AccessibilityProvider *LumeXComponentManager::provider_ = nullptr;
ArkUI_NativeNodeAPI_1 *nodeAPI = reinterpret_cast<ArkUI_NativeNodeAPI_1 *>(
    OH_ArkUI_QueryModuleInterfaceByName(ARKUI_NATIVE_NODE, "ArkUI_NativeNodeAPI_1"));
// [StartExclude plugin_manager_cpp]
LumeXComponentManager LumeXComponentManager::LumeXComponentManager_;
OH_NativeXComponent_Callback LumeXComponentManager::callback_;
ArkUI_NodeHandle xc;
int32_t LumeXComponentManager::hasDraw_ = 0;
int32_t LumeXComponentManager::hasChangeColor_ = 0;


LumeXComponentManager& LumeXComponentManager::LumeXComponentManager::GetInstance()
{
...
{
    OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "Callback", "~LumeXComponentManager");
    ...
    for (auto iter = LumeXComponentManagerMap_.begin(); iter != LumeXComponentManagerMap_.end(); ++iter) {
        if (iter->second != nullptr) {
            delete iter->second;
            iter->second = nullptr;
        }
    }
    LumeXComponentManagerMap_.clear();
}
...
void LumeXComponentManager::OnSurfaceCreatedNative(OH_ArkUI_SurfaceHolder *holder)
{
    ...
    LOGI("OnSurfaceCreatedNative: window=%{public}p, size=%{public}ux%{public}u", window, width, height);
    if(resourceMgr_ == nullptr){
        LOGI("resourceMgr_ is null");
    }
    if (renderer->Initialize(window, width, height,resourceMgr_)) {
...
{
    LOGI("OnSurfaceChangedNative: size=%{public}llu x %{public}llu", (unsigned long long)width, (unsigned long long)height);
...
void LumeXComponentManager::OnTouchEventNative(ArkUI_NodeEvent* event)
{
    ...
    LOGD("OnTouchEventNative: eventType=%{public}d", static_cast<int>(eventType));
...
napi_value LumeXComponentManager::LoadScene(napi_env env, napi_callback_info info)
{
    ...
    bool success = false;
    auto renderer = LumeXComponentManager::GetInstance().GetRendererById(nodeId);
    if (renderer) {
        LOGI("Renderer found");
...
LumeRenderer* LumeXComponentManager::GetRendererByNodeInternal(ArkUI_NodeHandle node)
{
    ...
    // LOGI("GetRendererByNodeInternal: node=%{public}p", it->second);
    if(it == rendererMap_.end()){
        LOGE("GetRendererByNodeInternal: node not found");
    }
...
LumeRenderer* LumeXComponentManager::GetRendererById(const std::string& id)
{
    ...
    LOGI("GetRendererById: id=%{public}s", id.c_str());
    auto it = nodeHandleMap_.find(id);
    if (it != nodeHandleMap_.end()) {
        LOGI("GetRendererById: node=%{public}p", it->second);
        return GetRendererByNodeInternal(it->second);
    }
    LOGI("GetRendererById: id=%{public}s not found", id.c_str());
    return nullptr;
}
// [StartExclude plugin_manager_cpp]
napi_value LumeXComponentManager::GetXComponentStatus(napi_env env, napi_callback_info info)
{
    napi_value hasDraw;
...
napi_value LumeXComponentManager::NapiDrawPattern(napi_env env, napi_callback_info info)
{
    // [StartExclude plugin_draw_pattern]
    OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "LumeXComponentManager", "NapiDrawPattern");
    if ((env == nullptr) || (info == nullptr)) {
        OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "LumeXComponentManager", "NapiDrawPattern: env or info is null");
        return nullptr;
    }
    ...
    if (napi_get_cb_info(env, info, nullptr, nullptr, &thisArg, nullptr) != napi_ok) {
        OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "LumeXComponentManager", "NapiDrawPattern: napi_get_cb_info fail");
        return nullptr;
    }
    
    // 调用绘制方法
    LumeXComponentManager::GetInstance().eglcore_->Draw(hasDraw_);
    OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "LumeXComponentManager", "render->eglCore_->Draw() executed");
...
void OnSurfaceCreatedCB(OH_NativeXComponent* component, void* window)
{
    ...
    // 初始化环境与绘制背景
    LumeXComponentManager::GetInstance().OnSurfaceCreated(component, window);
}
...
void OnSurfaceChangedCB(OH_NativeXComponent* component, void* window)
{
    ...
    LumeXComponentManager::GetInstance().OnSurfaceChanged(component, window);
}
...
void OnSurfaceDestroyedCB(OH_NativeXComponent* component, void* window)
{
    ...
    LumeXComponentManager::GetInstance().OnSurfaceDestroyed(component, window);
}
...
void DispatchTouchEventCB(OH_NativeXComponent* component, void* window)
{
    ...
    LumeXComponentManager::GetInstance().DispatchTouchEvent(component, window);
}
...
ArkUI_NodeHandle CreateNodeHandle(const std::string &tag)
{
    ...
    if (!nativeXComponent) {
        OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "LumeXComponentManager", "GetNativeXComponent error");
        return column;
    }
    OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "LumeXComponentManager", "GetNativeXComponent success");
    // 注册XComponent回调函数
    OH_NativeXComponent_RegisterCallback(nativeXComponent, &LumeXComponentManager::callback_);
    auto typeRet = nodeAPI->getAttribute(xc, NODE_XCOMPONENT_TYPE);
    OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "LumeXComponentManager", "xcomponent type: %{public}d",
                 typeRet->value[0].i32);
    auto idRet = nodeAPI->getAttribute(xc, NODE_XCOMPONENT_ID);
    OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "LumeXComponentManager", "xcomponent id: %{public}s",
                 idRet->string);
...
{
    if ((env == nullptr) || (info == nullptr)) {
        OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "LumeXComponentManager", "CreateNativeNode env or info is null");
        return nullptr;
    }
    size_t argCnt = 2;
    napi_value args[2] = { nullptr, nullptr };
    if (napi_get_cb_info(env, info, &argCnt, args, nullptr, nullptr) != napi_ok) {
        OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "LumeXComponentManager", "CreateNativeNode napi_get_cb_info failed");
    }
    if (argCnt != ARG_CNT) {
    ...
    OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "LumeXComponentManager", "tag=%{public}s", tag.c_str());
    int32_t ret = OH_ArkUI_NodeContent_SetUserData(nodeContentHandle_, new std::string(tag));
    if (ret != ARKUI_ERROR_CODE_NO_ERROR) {
        OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "LumeXComponentManager", "setUserData failed error=%{public}d", ret);
    }
    if (nodeAPI != nullptr && nodeAPI->createNode != nullptr && nodeAPI->addChild != nullptr) {
        OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "LumeXComponentManager",
...
{
    OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "XComponent_Native", "LumeXComponentManager::OnSurfaceCreated");
    int32_t ret;
    char idStr[OH_XCOMPONENT_ID_LEN_MAX + 1] = {};
    uint64_t idSize = OH_XCOMPONENT_ID_LEN_MAX + 1;
    ret = OH_NativeXComponent_GetXComponentId(component, idStr, &idSize);
...
{
    OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "XComponent_Native", "LumeXComponentManager::OnSurfaceDestroyed");
}
...
{
    if ((env == nullptr) || (info == nullptr)) {
        OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "LumeXComponentManager", "GetContext env or info is null");
        return nullptr;
    }
    ...
    if (napi_get_cb_info(env, info, &argCnt, args, nullptr, nullptr) != napi_ok) {
        OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "LumeXComponentManager", "GetContext napi_get_cb_info failed");
    }
...
void LumeXComponentManager::Export(napi_env env, napi_value exports)
{
    if ((env == nullptr) || (exports == nullptr)) {
        OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "LumeXComponentManager", "Export: env or exports is null");
        return;
    }

    napi_value exportInstance = nullptr;
    if (napi_get_named_property(env, exports, OH_NATIVE_XCOMPONENT_OBJ, &exportInstance) != napi_ok) {
        OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "LumeXComponentManager", "Export: napi_get_named_property fail");
        return;
    }

    OH_NativeXComponent* nativeXComponent = nullptr;
    if (napi_unwrap(env, exportInstance, reinterpret_cast<void**>(&nativeXComponent)) != napi_ok) {
        OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "LumeXComponentManager", "Export: napi_unwrap fail");
        return;
    }
    ...
    if (OH_NativeXComponent_GetXComponentId(nativeXComponent, idStr, &idSize) != OH_NATIVEXCOMPONENT_RESULT_SUCCESS) {
        OH_LOG_Print(
            LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "LumeXComponentManager", "Export: OH_NativeXComponent_GetXComponentId fail");
        return;
    }

    std::string id(idStr);
    if ((&LumeXComponentManager::GetInstance() != nullptr) && (nativeXComponent != nullptr)) {
        LumeXComponentManager::GetInstance().SetNativeXComponent(id, nativeXComponent);
        auto render = LumeXComponentManager::GetInstance().GetRender(id);
        if (render != nullptr) {
...
void LumeXComponentManager::SetNativeXComponent(std::string& id, OH_NativeXComponent* nativeXComponent)
{
...
PluginRender* LumeXComponentManager::GetRender(std::string& id)
{
...
napi_value LumeXComponentManager::BindNode(napi_env env, napi_callback_info info)
{
    ...
    LOGI("BindNode handle %{public}p", handle);
    if (!handle) {
    ...
    LOGI("BindNode holder %{public}p", holder);
    if (!holder) {
    ...
    LOGI("BindNode get resourceManger");
    resourceMgr_ = OH_ResourceManager_InitNativeResourceManager(env, args[2]); 
    if(resourceMgr_ == nullptr){
        LOGE("Failed to get resourceMgr");
    }
    ...
    if (!nodeAPI->addNodeEventReceiver(handle, OnTouchEventNative)) { // 添加事件监听，返回成功码 0
...
napi_value LumeXComponentManager::UnbindNode(napi_env env, napi_callback_info info)
{
    ...
    std::string nodeId = value2String(env, args[0]);
     LOGI("UnbindNode nodeId: %{public}s", nodeId.c_str());
    ...
    // if (LumeXComponentManager::callbackMap_.count(holder)) {
    //     auto callback = LumeXComponentManager::callbackMap_[holder];
    ...
    //     LumeXComponentManager::callbackMap_.erase(holder);
    // }
    // auto render = reinterpret_cast<EGLRender*>(OH_ArkUI_SurfaceHolder_GetUserData(holder));
    ...
    LOGI("UnbindNode completed: %{public}s", nodeId.c_str());
...
napi_value LumeXComponentManager::DrawStar(napi_env env, napi_callback_info info)
{
    size_t argc = 1;
    napi_value args[1] = {nullptr};
    napi_get_cb_info(env, info, &argc, args, nullptr, nullptr);
