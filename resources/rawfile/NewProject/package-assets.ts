...
function copyAssetFiles(dstRootPath: string, relativePath: string, filter: Function) {
    ...
    // Currently the scene files need a bit of extra processing for compatibility.
    processSceneFiles(fullDst);
}

function processSceneFile(sceneFilePath) {
    let data = fs.readFileSync(sceneFilePath, { encoding: 'utf8', flag: 'r' });

    const jsonData = JSON.parse(data);
    if (jsonData) {
        const meta = jsonData['$meta'];
        if (meta && meta['type'] === 'Scene' && meta['version'] === '0.1') {
            // Compatibility with older engine
            data = data.replace(/ae6addfc-5e8e-4c2d-8653-2bd1dca4be6c/g, 'f339e04b-317d-4527-8d3c-460467ff43da');
            fs.writeFileSync(sceneFilePath, data, { encoding: 'utf8' });
        }
    }
}

function processSceneFiles(currentPath) {
    if (fs.statSync(currentPath).isDirectory()) {
        const files = fs.readdirSync(currentPath);
        for (const fileName of files) {
            processSceneFiles(path.join(currentPath, fileName));
        }
    } else if (currentPath.endsWith('.scene')) {
        processSceneFile(currentPath);
    }
}
